...
def handle_send():
    def ambil_seperdelapan_pertama(data):
        n = len(data) // 10  # Hitung 1/5 dari panjang list
        return data[:n]  # Ambil 1/8 bagian pertama
    lema_list = df_kamus["LEMA"].tolist()
    lema_list_1per8 = ambil_seperdelapan_pertama(lema_list)

    user_input = st.session_state.user_input
    user_input = f"Hanya Gunakan kata kata ini: {lema_list_1per8}. Pertanyaan: {user_input}"
    st.success(f"prompt {len(user_input)}")
    st.success(f"prompt {user_input}")
    ...
    if fitur == "chatbot" and mode_bahasa == "Sunda":
        ...
        # Baris kode yang ingin dicatat waktunya
        ...
        st.success(f"✅ Waktu eksekusi: {elapsed_time:.2f} ms")
