...
def handle_send():
    ...
    # Get current room from session state
    current_room = st.session_state.get("room", "default")
    
    # Get previous messages from database
    history_data = get_chat_history(st.session_state.user.id, current_room)
    history_for_prompt = [{"message": msg["message"], "response": msg["response"]} 
...
if "user" in st.session_state:
    current_room = st.session_state.get("room", "default")
    chat_history = get_chat_history(st.session_state.user.id, current_room).data
    ...
    for chat in chat_history:
        ...
        if st.button(f"🗑️ Delete", key=f"delete_{chat['id']}"):
            delete_chat_message(chat['id'])
            st.rerun()
